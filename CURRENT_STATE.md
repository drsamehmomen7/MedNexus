...
Accepted checkpoints remain M0 `081e8b178f63850beb18f41a09494452aeacc4fc`, design `b64a8ee`, Radiology `31b22ab`, and MRJ identity/website baseline `9e96656`. The UNDERSTAND Single/Batch workspace is accepted. R1 PROTECT Journey Integration was **ACCEPTED AND CLOSED on 2026-09-22**, with a freshly verified baseline of **999 passed, 8 warnings, 0 failures**.

MRJ Website Experience / Cinematic Hero V1 is closed. The UNDERSTAND Single + Batch Journey Foundation and R1 PROTECT Journey Integration are also accepted and closed below. R1 extends that accepted workspace through PROTECT without changing the accepted privacy engine; later stages remain separately authorized. No naming exploration, logo redesign, or internal-identifier migration is authorized.
...
Roadmap: R0 architecture/contracts/UX (**accepted and closed**) → R1 PROTECT Single + Batch journey integration (**accepted and closed**) → R2.0A Clinical Fact/EXTRACT UX/evaluation specification (**accepted and closed**) → R2.0B Radiology Ground Truth Dataset (**in progress — cohort frozen / human annotation pending**) → R2.0C evaluation harness → R2.0D benchmark → R2.0E engine decision → R2 Radiology Clinical EXTRACT V0 (**not started**) → R3 Radiology STANDARDIZE V0 → R4 Radiology Collection V0 → R5 ANALYZE V0 → R6 VISUALIZE V0 → R7 INDICATORS V0 → real-report Radiology Horizontal Acceptance. MedGemma remains future-compatible and was not required for R1. The current verified regression is **1001 passed, 8 warnings, 0 failures**.

## R1 — PROTECT Journey Integration — Accepted and Closed

R1 extends the existing process-local `JourneyRun` rather than creating another privacy pipeline. Single and Batch reports now continue from UNDERSTAND to PROTECT in the same run. Each report owns independent PROTECT status, history, error and result state; eligible Batch reports are processed sequentially with failure isolation and the existing maximum of 10 reports. The accepted `DeidentificationService`, candidate-only OpenMed role, Intelligence Core, purpose-based Policy Engine and MRJ-owned final-output construction remain authoritative and unchanged. The standalone `/privacy` workflow remains available through its compatibility handoff.

The R0 output foundation is implemented as `ProtectedDocument`, `PatientAnalyticContext`, `ProtectionResult`, and `ProtectionProvenance`. `PatientAnalyticContext` is deliberately truthful and currently exposes no populated analytical fields: governed age/sex context is not available from the current engine, while age-band, safe-time, generalized-geography and pseudonymous-subject derivations are not implemented. Stored PROTECT results contain protected output and sanitized decision/provenance summaries, while the original `DocumentContent` remains only in bounded ephemeral server memory. No raw clinical content is written to browser storage or logs.

The accepted `/understanding` shell presents the PROTECT result in place, retains the report navigator for Batch review, updates real Journey Rail counts, and keeps EXTRACT visibly disabled. For PDF-origin reports the protected PDF is the primary document view; protected text is secondary, and Compare retrieves the unchanged retained original PDF only after explicit activation. MRJ builds each protected PDF deterministically from the authoritative protected text, keeps artifacts process-local, returns artifact metadata rather than bytes in Journey JSON, and marks both artifact responses non-cacheable. Switching reports or leaving Compare clears original-PDF browser references. Mixed Batch outcomes and artifacts remain independent, and one artifact-generation failure does not block other reports.

PROTECT readiness is now explicitly stage-specific. A readable, privacy-processable report may proceed from UNDERSTAND `NEEDS_REVIEW` with `UNKNOWN` or `OTHER` modality while preserving that review state; EXTRACT readiness remains separate and is not implied. The protection-completeness guard prevents a downstream-safe artifact when known high-risk labeled PHI remains unprotected. Review summaries expose only safe human labels and counts.

Human acceptance used 20 reports: **20 UNDERSTAND terminal, 20 PROTECT attempted, 20 protected PDF artifacts, 0 failed, 0 blocked, 0 known Patient Name/Patient ID/Visit ID misses, and 0 wrong clinical transformations**. Clinical content preservation and review explainability passed; the X-ray regression remained `FLUOROSCOPY`. All 20 PROTECT results were `NEEDS_REVIEW`, which is accepted for R1. Remaining future work is the high review burden; review-only ambiguous narrative person signals; Patient ID's current `DOCUMENT_ID` compatibility type plus provenance; unpopulated `PatientAnalyticContext`; absent age-band, date-shift and pseudonymous-subject derivations; absent OCR-specific privacy validation; and later improvement of UNKNOWN/OTHER recognition without blocking eligible privacy processing. Final focused UNDERSTAND/Journey tests passed **202 tests, 8 warnings**; protected-PDF artifact tests passed **5 tests, 1 warning**; privacy regression passed **272 tests, 7 warnings**; full regression passed **999 tests, 8 warnings, 0 failures** in **122.35s**. JavaScript and Python validation passed. **R1 was accepted and closed on 2026-09-22. R2 runtime implementation has not started.**

## R2.0A — Radiology Clinical Fact Contract + EXTRACT UX Contract — Accepted and Closed

R2.0A was formally **ACCEPTED AND CLOSED** on **2026-09-24**. The authoritative package consists of [Radiology Clinical Fact Contract v1.0](docs/contracts/MRJ_Radiology_Clinical_Fact_Contract_v1.0.md), [Radiology EXTRACT UX Contract v1.0](docs/design/MRJ_Radiology_EXTRACT_UX_Contract_v1.0.md), and [R2.0 Radiology Extraction Evaluation Plan v1.0](docs/evaluation/MRJ_R2_0_Radiology_Extraction_Evaluation_Plan_v1.0.md).
