...
R2.0A — Radiology Clinical Fact Contract + EXTRACT UX Contract was accepted and closed on 2026-09-24. `ClinicalContextFact` may preserve source-supported indication, symptom, history/comorbidity, event/exposure, or other clinical context, but it must remain privacy-safe, must not rediscover identity removed by PROTECT, and must not become a current `RadiologyFinding` without governed evidence. Report-level facts and co-occurrence do not establish cohort-level conclusions or causation.
...
The next checkpoint is R2.0B — Radiology Ground Truth Dataset, followed by R2.0C Evaluation Harness, R2.0D Benchmark, R2.0E Engine Decision, and R2 — Radiology Clinical EXTRACT V0; none has started. MedGemma remains future-compatible and was not required for R1. Documentation synchronization remains mandatory after every meaningful checkpoint.
