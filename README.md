...
The clinical EXTRACT engine has not started; no clinical extraction API, dependency, model, evaluation harness, benchmark, engine selection, STANDARDIZE, or analytics capability is implemented. **R2.0B — Radiology Ground Truth Dataset is IN PROGRESS — COHORT FROZEN / SECTION-AWARE FRAMEWORK HUMAN-REVIEWED AND ACCEPTED / HUMAN CLINICAL ANNOTATION PENDING.** Its [cohort-governance record](docs/evaluation/MRJ_R2_0B_Ground_Truth_Cohort_v1.0.md) documents the external 24-report protected canonical-input workspace and empty annotation framework. Protected reports, canonical clinical text, and annotation work products remain outside Git. R2.0C Evaluation Harness, R2.0D Benchmark, R2.0E Engine Decision, and R2 Radiology Clinical EXTRACT V0 have not started.

## R2 EXTRACT UI Shell — Human Accepted

The existing Clinical Journey Workspace now has a human-accepted EXTRACT review shell. The Protected Report remains primary beside a two-column desktop clinical workspace with Findings, Relationships, and Clinical Context views, collapsed Technical Details, evidence-navigation hooks, and presentation support for clinical salience, assertion states, and internal report conflict. `NOT_RUN` shows no result counts; a genuine completed zero-result extraction may show zero. Safe protected input may remain available when PROTECT is `NEEDS_REVIEW`, while protection completeness failure still blocks unsafe downstream use. The shell displays no invented clinical facts. **No extraction engine or model is installed; clinical extraction is not yet functional.**
