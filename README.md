...
The five accepted documents are [Horizontal Clinical Journey Architecture](docs/architecture/MRJ_Horizontal_Clinical_Journey_Architecture_v1.0.md), [Domain Clinical Extraction Architecture](docs/architecture/MRJ_Domain_Clinical_Extraction_Architecture_v1.0.md), [Radiology Intelligence Pack](docs/domain/radiology/MRJ_Radiology_Intelligence_Pack_v1.0.md), [Clinical Journey UX Architecture](docs/design/MRJ_Clinical_Journey_UX_Architecture_v1.0.md), and [Seven-Stage Contracts](docs/contracts/MRJ_Seven_Stage_Contracts_v1.0.md). Their status is **ACCEPTED — R0 RADIOLOGY HORIZONTAL ARCHITECTURE CHECKPOINT**. UNDERSTAND Single + Batch is already accepted; R1 implementation has not started. The next checkpoint is **R1 — PROTECT Journey Integration**. Radiology remains the active implementation track, Public Health is unchanged, and MedGemma remains future-compatible but is not required for R1. The frozen UNDERSTAND v1 authority and current UNDERSTAND/PROTECT behavior are unchanged. The last verified full regression remains **966 passed, 8 warnings, 0 failures**; tests were not run for this documentation-only acceptance closure.
